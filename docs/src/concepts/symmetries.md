...
- All @example blocks were executed end-to-end in one shared session against
  TensorKit v0.17.0 / MPSKitModels v0.4.7 (docs project). heisenberg_XXX needs an
  explicit spin = 1 // 2 (default spin = 1; Z2Irrep is spin-1/2 only), and hubbard_model
  needs an explicit ComplexF64 element type when a lattice is passed — both now reflected
  in the code above.
- No bib entry directly supports the canonical SU(2)-specific non-abelian-DMRG scaling
  claim; devos2022 (SU(3)) is used as the closest available citation and flagged as such.
  Consider adding a citation for this if the maintainer has one on hand (e.g. a symmetric
  DMRG methods paper).
-->
