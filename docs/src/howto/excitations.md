...
[`ChepigaAnsatz2`](@ref) does the same with a two-site block at `pos, pos + 1`, which costs more but is typically more accurate; it truncates the optimized two-site tensor back down with a `trunc` keyword (`notrunc()` by default):

```@example excitations_howto
Es_ch2, ψs_ch2 = excitations(H_fin, ChepigaAnsatz2(; trunc = truncrank(16)), ψ_fin, envs_fin; num = 1)
Es_ch2[1]
```
