...
function iscached(root, name)
    cache = getcache()
    return haskey(cache, root) && haskey(cache[root], name) && 
...
function setcached(root, name)
    ...
    open(f -> TOML.print(f, cache), CACHEFILE, "w")
end
...
function attach_notebook_badge(root, name, str)
    ...
    download = "[![]($download_badge_url)](https://minhaskamal.github.io/DownGit/#/home?url=https://github.com/maartenvd/MPSKit.jl/tutorials/tree/gh-pages/dev/examples/$root/$name)"
...
function build_example(root, name)
    ...
    if !iscached(root, name)
        Literate.markdown(source_file, target_dir; execute=true, name="index",
                        ...
                        nbviewer_root_url="https://nbviewer.jupyter.org/github/maartenvd/MPSKit.jl/blob/gh-pages/dev/", binder_root_url="https://mybinder.org/v2/gh/maartenvd/MPSKit.jl/gh-pages?filepath=dev/", credits=false)
        Literate.notebook(source_file, target_dir; execute=false, name="main",
                        preprocess=str -> replace(str, r"(?<!`)``(?!`)" => "\$"), mdstrings=true, credits=false)
