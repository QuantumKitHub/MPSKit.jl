...
function XY_hamiltonian(
        ::Type{TorA} = ComplexF64, ::Type{S} = Trivial; J = 1 / 2, N
    ) where {TorA, S <: Sector}
    spin = 1 // 2
    term = J * (S_xx(TorA, S; spin) + S_yy(TorA, S; spin))
...
function groundstate_energy(J, N)
    ...
    T = CuArray(diagm(1 => J / 2 * ones(N - 1), -1 => J / 2 * ones(N - 1)))
...
N_exact = 6
H = open_boundary_conditions(adapt(CuArray, XY_hamiltonian(ComplexF64, symmetry; J, N = Inf)), N_exact)
...
N = 32
H = adapt(CuArray, XY_hamiltonian(ComplexF64, symmetry; J, N))
D = 64
V_init = symmetry === Trivial ? ℂ^32 : U1Space(i => 10 for i in -1:(1 // 2):1)
psi_init = adapt(CuArray, FiniteMPS(N, physicalspace(H, 1), V_init))
