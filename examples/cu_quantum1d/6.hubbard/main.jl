...
H = adapt(CuArray, hubbard_model(InfiniteChain(2); U, t, mu = U / 2))
Vspaces = fill(Vect[fℤ₂](0 => 10, 1 => 10), 2)
psi = adapt(CuArray, InfiniteMPS(physicalspace(H), Vspaces))
...
H_u1_su2 = adapt(CuArray, MPSKit.add_physical_charge(H_u1_su2, charges));

pspaces = physicalspace.(Ref(H_u1_su2), 1:2)
vspaces = [oneunit(eltype(pspaces)), first(pspaces)]
psi = adapt(CuArray, InfiniteMPS(pspaces, vspaces))
