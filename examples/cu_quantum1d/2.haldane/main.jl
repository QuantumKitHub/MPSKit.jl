...
using MPSKit, MPSKitModels, TensorKit, Plots, Polynomials, MatrixAlgebraKit
...
L = 11
chain = FiniteChain(L)
H = adapt(CuArray, heisenberg_XXX(symmetry, chain; J, spin))

physical_space = SU2Space(1 => 1)
virtual_space = SU2Space(0 => 12, 1 => 12, 2 => 5, 3 => 3)
ψ₀ = adapt(CuArray, FiniteMPS(L, physical_space, virtual_space))
...
SS = adapt(CuArray, -S_exchange(ComplexF64, SU2Irrep; spin = 1))
...
ΔEs = map(Ls) do L
    @info "computing L = $L"
    ψ₀ = adapt(CuArray, FiniteMPS(L, physical_space, virtual_space))
    H = adapt(CuArray, heisenberg_XXX(symmetry, FiniteChain(L); J, spin))
    ψ, envs, delta = find_groundstate(ψ₀, H, DMRG(; verbosity = 0))
    En_1, st_1 = excitations(H, QuasiparticleAnsatz(), ψ, envs; sector = SU2Irrep(1))
    En_2, st_2 = excitations(H, QuasiparticleAnsatz(), ψ, envs; sector = SU2Irrep(2))
...
ψ₀_inf = adapt(CuArray, InfiniteMPS([physical_space], [virtual_space_inf]))
MPSKit.Defaults.alg_qr() = CUSOLVER_HouseholderQR(; positive = true)
MPSKit.Defaults.alg_svd() = CUSOLVER_QRIteration()
MPSKit.Defaults.alg_lq() = LQViaTransposedQR(CUSOLVER_HouseholderQR(; positive = true))
