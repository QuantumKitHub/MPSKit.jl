using Markdown
using Adapt, CUDA, cuTENSOR, MPSKit, MPSKitModels, TensorKit
...
cutoff, D = 4, 5
initial_state = adapt(CuArray, InfiniteMPS(ℂ^(cutoff + 1), ℂ^D))
...
hamiltonian = adapt(CuArray, bose_hubbard_model(InfiniteChain(1); cutoff = cutoff, U = 1, mu = 0.5, t = 0.2)) # It is not strictly required to pass InfiniteChain() and is only included for clarity; one may instead pass FiniteChain(N) as well
...
function get_ground_state(mu, t, cutoff, D; kwargs...)
    hamiltonian = adapt(CuArray, bose_hubbard_model(InfiniteChain(); cutoff = cutoff, U = 1, mu = mu, t = t))
    state = adapt(CuArray, InfiniteMPS(ℂ^(cutoff + 1), ℂ^D))
