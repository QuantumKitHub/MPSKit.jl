...
@testset "Exact diagonalization" begin
    ...
    @test isapprox(E, [-3, -1, 1, 3]; atol=1e-6)
end
...
@testset "Finite systems" begin
    ...
    ψ2, envs2 = find_groundstate(init, H, DMRG2(;trscheme=truncbelow(1e-6)))
    E2 = expectation_value(ψ2, H, envs2)

    @test isapprox(E, E2; atol=1e-6)

    ED, _ = exact_diagonalization(H; sector=D0)
    @test isapprox(E, first(ED); atol=1e-6)
...
@testset "Infinite systems" begin
    ...
    ψ2, envs2 = find_groundstate(init, H, IDMRG2(; trscheme=truncbelow(1e-9)))
    E2 = expectation_value(ψ2, H, envs2)
