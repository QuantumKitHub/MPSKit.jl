...
@testset "Finite MPO-MPS zipper $(spacetype(pspace))" for (pspace, Dspace) in spacelist
    ...
    Dcut = 4
    got_tr = zipper(O, ψ, truncrank(Dcut))
    @test maximum(i -> dim(left_virtualspace(got_tr, i)), 2:length(got_tr)) <= Dcut
end

@testset "Density-matrix FiniteMPS zipper $(spacetype(pcomp))" for (pcomp, Dspace) in [
        (ℙ^2 ⊗ (ℙ^2)', ℙ^6),
    ...
    Vspaces = [oneunit(Wspace); fill(Wspace, L - 1); oneunit(Wspace)]
    O = FiniteMPO(
        [rand(ComplexF64, Vspaces[i] ⊗ pcomp ← pcomp ⊗ Vspaces[i + 1]) for i in 1:L]
    )
