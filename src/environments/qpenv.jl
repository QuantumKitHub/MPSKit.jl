...
function gen_exci_lw_rw(left_gs::Union{FiniteMPS{A},InfiniteMPS{A}},
    ...
    for j in 1:size(lw, 1), i in 1:size(lw, 2)
        lw[j, i] = fill_data!(similar(left_gs.AL[1],
                                      ...
                                      ham[i].domspaces[j]',
                                      excileg' * right_virtualspace(right_gs, i - 1)), zero)
        rw[j, i] = fill_data!(similar(left_gs.AL[1],
                                      left_virtualspace(left_gs, i) * ham[i].imspaces[j]',
                                      excileg' * right_virtualspace(right_gs, i)), zero)
    end
...
function environments(exci::InfiniteQP, ham::MPOHamiltonian, lenvs, renvs;
    ...
    for i in length(exci):-1:2
        ...
        exci.trivial && for k in ids
                        @plansor rB_cur[k][-1 -2; -3 -4] -= τ[6 4; 1 3] * rB_cur[k][1 3; -3 2] *
...
function environments(exci::FiniteQP, ham::MPOHamiltonian,
...
function environments(exci::Multiline{<:InfiniteQP}, ham::MPOMultiline, lenvs, renvs;
                      solver=Defaults.linearsolver)
    exci.trivial ||
    ...
    for row in 1:numrows
        ...
        for col in 1:numcols
            ...
            left_renorms[col] = @plansor lv[1 2; 3] * left_above.AC[col][3 4; 5] *
                                         hamrow[col][2 6; 4 7] * rv[5 7; 8] *
                                         conj(left_below.AC[col][1 6; 8])

            lv = leftenv(renvs, col, right_gs)[row]
            rv = rightenv(renvs, col, right_gs)[row]
            right_renorms[col] = @plansor lv[1 2; 3] * right_above.AC[col][3 4; 5] *
                                          hamrow[col][2 6; 4 7] * rv[5 7; 8] *
