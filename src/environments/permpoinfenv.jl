...
function gen_init_fps(above::MPSMultiline, mpo::Multiline{<:DenseMPO}, below::MPSMultiline)
    ...
    map(1:size(mpo, 1)) do cr
        L0::T = randomize!(
            similar(
                above.AL[1, 1],
                left_virtualspace(below, cr + 1, 0) * _firstspace(mpo[cr, 1])',
        ...
        R0::T = randomize!(
            similar(
                above.AL[1, 1],
                right_virtualspace(above, cr, 0) * _firstspace(mpo[cr, 1]),
...
function gen_init_fps(above::MPSMultiline, mpo::Multiline{<:SparseMPO}, below::MPSMultiline)
    map(1:size(mpo, 1)) do cr
        ...
        for j in 1:(ham.odim)
            lw[j] = similar(
                ab.AL[1],
                _firstspace(be.AL[1]) * ham[1].domspaces[j]',
                _firstspace(ab.AL[1]),
            )
            rw[j] = similar(
                ab.AL[1],
                _lastspace(ab.AR[end])' * ham[end].imspaces[j]',
...
function mixed_fixpoints(
    above::MPSMultiline,
    mpo::MPOMultiline,
    below::MPSMultiline,
    init=gen_init_fps(above, mpo, below);
    solver=Defaults.eigsolver,
)
    T = eltype(above)
    ...
    @threads for cr in 1:numrows
        ...
        for loc in (numcols - 1):-1:1
            righties[cr, loc] =
                TransferMatrix(c_above.AR[loc + 1], mpo[cr, loc + 1], c_below.AR[loc + 1]) *
                righties[cr, loc + 1]

            renormfact = dot(
                c_below.CR[loc],
                MPO_∂∂C(lefties[cr, loc + 1], righties[cr, loc]) * c_above.CR[loc],
            )
