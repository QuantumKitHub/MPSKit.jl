...
function gen_lw_rw(st::InfiniteMPS{A}, ham::Union{SparseMPO,MPOHamiltonian}) where {A}
    ...
    for i in 1:length(st), j in 1:(ham.odim)
        lw[j, i] = similar(st.AL[1],
                           _firstspace(st.AL[i]) * ham[i].domspaces[j]' ←
                           _firstspace(st.AL[i]))
        rw[j, i] = similar(st.AL[1],
                           _lastspace(st.AR[i])' * ham[i].imspaces[j]' ←
                           _lastspace(st.AR[i])')
    end
    ...
    for i in 1:size(fp, 2)
        ...
        for j in index:-1:1
            ...
            if isscal(ham[i], j, index)
                ...
                axpy!(true, fp[j, i] * TransferMatrix(st.AL[i], ham[i][j, index], st.AL[i]),
                      fp[index, i + 1])
            end
        end
    end
    ...
    for i in size(fp, 2):(-1):1
        ...
        for j in index:size(fp, 1)
            ...
            if isscal(ham[i], index, j)
                ...
                axpy!(true, TransferMatrix(st.AR[i], ham[i][index, j], st.AR[i]) * fp[j, i],
                      fp[index, i - 1])
            end
        end
    end
