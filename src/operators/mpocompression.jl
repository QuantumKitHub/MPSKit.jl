"""
    mpo_compression(H::FiniteMPOHamiltonian; truncbelow::Number=10^-8) -> Hᵪ, Rs
    mpo_compression(H::InfiniteMPOHamiltonian; truncbelow::Number=10^-8) -> Qs, Ps
...
### Keyword arguments
- `truncbelow::Number=10^-8`: precision of the compression. The values smaller than truncbelow in the SVD 
  decomposition during compression will be disregarded. 

### Returns
- mpo_compression(H::FiniteMPOHamiltonian; truncbelow::Number=10^-8)
...
- mpo_compression(H::InfiniteMPOHamiltonian; truncbelow::Number=10^-8)
...
function mpo_compression(H::FiniteMPOHamiltonian; truncbelow::Number = 10^-8, verbosity::Int = 0)
    t = @elapsed begin
        ...
        for n in 2:(N - 1)
            ...
            M, R′ = decompose_R(R)
            U, S, V′ = block_respecting_svd(M, truncbelow)
...
function mpo_compression(H::InfiniteMPOHamiltonian; truncbelow::Number = 10^-8, verbosity::Int = 0, canonical_precision = 1.0e-10)
    tot_dim_original = total_virt_dimension(H)
    t = @elapsed begin
        ...
        HL, _ = left_canonical_mpo_infinite_iter_msites(Hnew; precision = canonical_precision, verbosity)
        _, HR = right_canonical_mpo_infinite_iter_msites(HL; precision = canonical_precision, verbosity)
        HL, Cs = left_canonical_mpo_infinite_iter_msites(HR; precision = canonical_precision, verbosity)
        Us = Vector{BlockTensorMap}(undef, N)
        Ss = Vector{BlockTensorMap}(undef, N)
        Vs = Vector{BlockTensorMap}(undef, N)
        for n in 1:N
            Us[n], Ss[n], Vs[n] = block_respecting_svd(Cs[n]; truncbelow)
        end
...
function block_respecting_svd(M::AbstractBlockTensorMap, truncbelow::Number; perform_truncation = true)
    # Perform block-respecting svd on matrix M, with precision truncbelow
