function left_canonicalize!(
    ...
    if size(W, 1) == 1
        ...
        if dim(R) == 0 # fully truncated
            ...
            V = ⊞(unitspace(S), space(R, 1), unitspace(S))
            scale!(Q, d)
            scale!(R, inv(d))
        ...
        if dim(R) == 0 # fully truncated
            ...
            V = ⊞(unitspace(S), right_virtualspace(Q′), unitspace(S))
        end
        H[i] = JordanMPOTensor(codomain(W) ← physicalspace(W) ⊗ V, Q1, W.B, Q2, W.D)
    end
...
function right_canonicalize!(
    ...
    if size(W, 4) == 1
        ...
        if dim(R) == 0
            ...
            V = ⊞(unitspace(S), space(Q, 1), unitspace(S))
            scale!(Q, d)
            scale!(R, inv(d))
        ...
        if dim(R) == 0
            ...
            V = ⊞(unitspace(S), left_virtualspace(Q′), unitspace(S))
        end
        H[i] = JordanMPOTensor(V ⊗ physicalspace(W) ← domain(W), Q1, Q2, W.C, W.D)
    end
