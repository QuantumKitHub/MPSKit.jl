...
r_RR(state::FiniteMpo{T}) where T = TensorMap(I,eltype(T),space(state[end],3)',space(state[end],3)')
l_LL(state::FiniteMpo{T}) where T = TensorMap(I,eltype(T),space(state[1],1),space(state[1],1))

function expectation_value(ts::FiniteMpo,opp::TensorMap)
    leftenvs = [Tensor(I,ComplexF64,space(ts[1],1)')];
    rightenvs = [Tensor(I,ComplexF64,space(ts[length(ts)],3)')];

    for i in 1:length(ts)
        #we can't trace :(
        lefttracer = TensorMap(I,space(ts[i],2)',space(ts[i],2)')
        @tensor curl[-1] := leftenvs[end][1]*ts[i][1,2,-1,3]*lefttracer[2,3]
        push!(leftenvs,curl);

        righttracer = TensorMap(I,space(ts[length(ts)-i+1],2)',space(ts[length(ts)-i+1],2)')
