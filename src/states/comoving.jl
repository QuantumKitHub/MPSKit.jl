...
mutable struct MPSComoving{A<:GenericMPSTensor,B<:MPSBondTensor} <: AbstractFiniteMPS
    ...
    function MPSComoving(left::InfiniteMPS{A,B}, window::FiniteMPS{A,B},
                         right::InfiniteMPS{A,B}) where {A<:GenericMPSTensor,
                                                         B<:MPSBondTensor}
        #todo:insert checks
...
function MPSComoving(left::InfiniteMPS, site_tensors::Array, right::InfiniteMPS)
    return MPSComoving(left, FiniteMPS(site_tensors), right)
end
...
function MPSComoving(f, elt, physspaces::Vector{<:Union{S,CompositeSpace{S}}},
                     maxvirtspace::S,
                     leftgs::M,
    ...
    return MPSComoving(leftgs, window, rightgs)
end
function MPSComoving(f, elt,
    ...
    tensors = [TensorMap(f, elt, virtspaces[n] ⊗ physspaces[n], virtspaces[n + 1])
               for n in 1:N]
...
function MPSComoving(state::InfiniteMPS{A,B}, len::Int) where {A,B}
    ...
    return MPSComoving(state, FiniteMPS(ALs, ARs, ACs, CLs), state)
end

function Base.copy(state::MPSComoving{A,B}) where {A,B}
    return MPSComoving(state.left_gs, copy(state.window), state.right_gs)
end;

Base.length(state::MPSComoving) = length(state.window)
Base.size(psi::MPSComoving, i...) = size(psi.window, i...)

function Base.eltype(::Type{MPSComoving{Mtype,Vtype}}) where {Mtype<:GenericMPSTensor,
                                                              Vtype<:MPSBondTensor}
    return Mtype
end
function site_type(::Type{MPSComoving{Mtype,Vtype}}) where {Mtype<:GenericMPSTensor,
                                                            Vtype<:MPSBondTensor}
    return Mtype
end
function bond_type(::Type{MPSComoving{Mtype,Vtype}}) where {Mtype<:GenericMPSTensor,
                                                            Vtype<:MPSBondTensor}
    return Vtype
end
