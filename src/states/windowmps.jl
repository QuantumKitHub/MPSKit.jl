...
"
const WindowMPS{A,B} = Window{InfiniteMPS{A,B},FiniteMPS{A,B},InfiniteMPS{A,B}} where {A<:GenericMPSTensor,B<:MPSBondTensor}
...
function WindowMPS(left::InfiniteMPS{A,B},window::FiniteMPS{A,B},right::InfiniteMPS{A,B}) where {A<:GenericMPSTensor,B<:MPSBondTensor}
    #todo:insert checks
    return WindowMPS{A,B}(left,window,right);
end
