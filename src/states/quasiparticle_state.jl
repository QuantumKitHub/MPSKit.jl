...
#constructors
function LeftGaugedQP(datfun, left_gs, right_gs=left_gs; sector=one(sectortype(left_gs)),
                      momentum=0.0)
    ...
    Xs = [TensorMap(datfun, eltype(left_gs.AL[1]), _lastspace(VLs[loc])',
                    excitation_space' * right_virtualspace(right_gs, loc))
          for loc in 1:length(left_gs)]
    left_gs isa InfiniteMPS || momentum == zero(momentum) ||
        @warn "momentum is ignored for finite quasiparticles"
    return LeftGaugedQP(left_gs, right_gs, VLs, Xs, momentum)
end

function RightGaugedQP(datfun, left_gs, right_gs=left_gs; sector=one(sectortype(left_gs)),
                       momentum=0.0)
    #find the left null spaces for the TNS
    excitation_space = ℂ[typeof(sector)](sector => 1)
    VRs = [adjoint(leftnull(adjoint(v))) for v in _transpose_tail.(right_gs.AR)]
    Xs = [TensorMap(datfun, eltype(left_gs.AL[1]), left_virtualspace(right_gs, loc - 1)',
