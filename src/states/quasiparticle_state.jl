...
Base.getindex(v::RightGaugedQP,i::Int) = @plansor t[-1 -2;-3 -4] := v.Xs[mod1(i,end)][-1;-3 1]*v.VRs[mod1(i,end)][1;-4 -2];
...
function Base.convert(::Type{RightGaugedQP},input::LeftGaugedQP{S}) where S<:InfiniteMPS
    ...
    for i in 1:len
        @plansor T[-1 -2;-3 -4] := input.left_gs.AL[i][-1 -2;1]*rBs[mod1(i+1,end)][1;-3 -4]
...
function Base.convert(::Type{LeftGaugedQP},input::RightGaugedQP{S}) where S<:InfiniteMPS
    ...
    for i in 1:len
        @plansor T[-1 -2;-3 -4] := lBs[mod1(i-1,len)][-1;-3 1]*input.right_gs.AR[i][1 -2;-4]
        @plansor T[-1 -2;-3 -4] += input[i][-1 -2;-3 -4]
        lg[i] = T
    end
...
function Base.convert(::Type{<:FiniteMPS},v::QP{S}) where S <: FiniteMPS
    ...
    function simplefuse(temp)
        ...
        @plansor tempp[-1 -2;-3] := frontmap[-1;1 2]*temp[1 2 -2 3;4]*backmap[4 3;-3]
    end
