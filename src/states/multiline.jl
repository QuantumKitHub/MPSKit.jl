...
function MPSMultiline(pspaces::AbstractArray{S,2}, Dspaces::AbstractArray{S,2};
                      kwargs...) where {S}
    MPSMultiline(map(zip(circshift(Dspaces, (0, -1)), pspaces, Dspaces)) do (D1, p, D2)
                     return TensorMap(rand, Defaults.eltype, D1 * p, D2)
                 end; kwargs...)
end
...
function MPSMultiline(data::AbstractArray{T,2}; kwargs...) where {T<:GenericMPSTensor}
    Multiline(PeriodicArray(map(1:size(data, 1)) do row
                                return InfiniteMPS(data[row, :]; kwargs...)
                            end))
end
function MPSMultiline(data::AbstractArray{T,2}, Cinit::AbstractArray{O,1};
                      kwargs...) where {T<:GenericMPSTensor,O<:MPSBondTensor}
    Multiline(PeriodicArray(map(1:size(data, 1)) do row
                                return InfiniteMPS(data[row, :], Cinit[row]; kwargs...)
                            end))
end
