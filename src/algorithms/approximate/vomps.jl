...
function approximate(state::InfiniteMPS,
                     toapprox::Tuple{<:Union{SparseMPO,DenseMPO},<:InfiniteMPS}, alg,
    ...
    (multi, envs) = approximate(convert(MPSMultiline, state),
                                (convert(MPOMultiline, envs.opp),
                                 convert(MPSMultiline, envs.above)), alg, envs)
    state = convert(InfiniteMPS, multi)
    return (state, envs)
end
function approximate(state::MPSMultiline, toapprox::Tuple{<:MPOMultiline,<:MPSMultiline},
                     alg::VUMPS, envs=environments(state, toapprox))
    (mpo, above) = toapprox
    ...
    while true
        ...
        state = MPSMultiline(temp_ACs, state.CR[:, end]; tol=alg.tol_gauge,
