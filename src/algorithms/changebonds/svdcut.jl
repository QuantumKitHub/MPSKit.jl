...
function changebonds(state::Union{FiniteMps{T},MpsComoving{T}},alg::SvdCut) where T<: GenMpsType{Sp,N} where {Sp,N} # made it work for GenMpsType
    ...
    for i in length(state)-1:-1:1
        a = state[i]#permuteind(state[i],ntuple(x->x,Val{N}()),(N+1,));
        b = permuteind(state[i+1],(1,),ntuple(x->x+1,Val{N}()));

        (U,S,V) = svd(a*b,trunc=alg.trschemes[mod1(i,end)],alg=TensorKit.SVD());

        state[i] = U*S;
        state[i+1] = permuteind(V,ntuple(x->x,Val{N}()),(N+1,))
    end
...
function changebonds(state::MpsCenterGauged,alg::SvdCut)
    ...
    for i in 1:length(state)
        (U,S,V) = svd(state.CR[i],trunc=alg.trschemes[mod1(i,end)],alg=TensorKit.SVD());
