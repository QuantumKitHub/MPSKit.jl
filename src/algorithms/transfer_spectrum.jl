...
- `krylovdim`: the Krylov dimension of the eigensolver. Unless given explicitly, this is chosen adaptively per sector as
  `max(Defaults.krylovdim, ceil(Int, oversampling_factor * howmany) + oversampling)`, where `howmany` is the number of values requested in that sector.
...
function transfer_spectrum(
    ...
    howmany = _transfer_howmany(howmany, transferspace)
    ...
    for (c, n) in howmany
        algorithm = MatrixAlgebraKit.select_algorithm(
            transfer_spectrum, above, alg; howmany = n, kwargs...
        )
        Va = S(c => 1)
...
# normalize the `howmany` specification into a `SectorDict` of per-sector counts,
# capped by the transfer-space dimension in each sector
_transfer_howmany(howmany::Int, V) =
    TensorKit.SectorDict(c => min(howmany, dim(V, c)) for c in sectors(V))
function _transfer_howmany(howmany, V)
    return TensorKit.SectorDict(
        c => min(n, dim(V, c)) for (c, n) in howmany if dim(V, c) > 0
    )
end

# `howmany` is intercepted here so it never reaches the algorithm constructor, and is
...
function MatrixAlgebraKit.select_algorithm(
        ...
        alg = nothing; howmany = 20, kwargs...
    )
    if isnothing(alg)
        return MatrixAlgebraKit.default_algorithm(transfer_spectrum, above; howmany, kwargs...)
    elseif alg isa NamedTuple || alg isa Base.Pairs
        ...
        return MatrixAlgebraKit.default_algorithm(transfer_spectrum, above; alg..., howmany)
    elseif alg isa DefaultAlgorithm
        isempty(kwargs) ||
            throw(ArgumentError("keyword arguments are not allowed when `alg` is given"))
        return MatrixAlgebraKit.default_algorithm(transfer_spectrum, above; alg.kwargs..., howmany)
...
function MatrixAlgebraKit.default_algorithm(
        ::typeof(transfer_spectrum), ::Type{<:InfiniteMPS}; howmany = 20,
        tol = Defaults.tol, maxiter = Defaults.maxiter,
        oversampling = 10, oversampling_factor = 1,
        krylovdim = max(Defaults.krylovdim, ceil(Int, oversampling_factor * howmany) + oversampling),
...
"""
function marek_gap(above::InfiniteMPS; sector = nothing, tol_angle = 0.1, howmany = 20, kwargs...)
    if isnothing(sector)
        spectrum = transfer_spectrum(above; howmany, kwargs...)
        return TensorKit.SectorDict(c => marek_gap(spectrum[c]; tol_angle) for c in keys(spectrum))
    end
    spectrum = transfer_spectrum(above; howmany = Dict(sector => howmany), kwargs...)
