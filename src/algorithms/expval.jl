...
function expectation_value(state::Union{InfiniteMPS,MPSComoving,FiniteMPS},
...
function expectation_value(state::Union{InfiniteMPS,MPSComoving,FiniteMPS},
    ...
    map(zip(state.AC, opps)) do (ac, opp)
        return tr(ac' * transpose(opp * transpose(ac, (TensorKit.allind(ac)[2:(end - 1)]),
...
"""
function expectation_value(state::Union{FiniteMPS{T},MPSComoving{T},InfiniteMPS{T}},
...
function expectation_value(state::Union{FiniteMPS{T},MPSComoving{T},InfiniteMPS{T}},
    ...
    @plansor v[-1 -2; -3] := isomorphism(storagetype(T), left_virtualspace(state, at - 1),
                                         left_virtualspace(state, at - 1))[-1; -3] *
                             conj(ut[-2])
    tmp = v * TransferMatrix(state.AL[at:(at + length(op) - 1)], op,
                             state.AL[at:(at + length(op) - 1)])
    return @plansor tmp[1 2; 3] * ut[2] * state.CR[at + length(op) - 1][3; 4] *
...
function expectation_value_fimpl(state::AbstractFiniteMPS, ham::MPOHamiltonian,
    ...
    for i in 1:length(state),
        ...
        cur = @plansor leftenv(envs, i, state)[j][1 2; 3] * state.AC[i][3 7; 5] *
                       rightenv(envs, i, state)[k][5 8; 6] * conj(state.AC[i][1 4; 6]) *
                       ham[i][j, k][2 4; 7 8]
        if !(j == 1 && k == ham.odim)
            cur /= 2
        end
...
function expectation_value(st::InfiniteMPS, ham::MPOHamiltonian, range::Int64,
                           prevca=environments(st, ham))
    return expectation_value(st, ham, 1:range, prevca)
end
function expectation_value(st::InfiniteMPS, ham::MPOHamiltonian, range::UnitRange{Int64},
                           prevca=environments(st, ham))
