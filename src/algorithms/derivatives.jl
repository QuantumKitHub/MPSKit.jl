...
function ∂AC(x::MPSTensor, ham::SparseMPOSlice, leftenv, rightenv)::typeof(x)
    ...
    @floop WorkStealingEx() for (i, j) in keys(ham)
        if isscal(ham, i, j)
            @plansor t[-1 -2; -3] := leftenv[i][-1 5; 4] * x[4 6; 1] * τ[6 5; 7 -2] *
                                     rightenv[j][1 7; -3]
            lmul!(ham.Os[i, j], t)
        else
            @plansor t[-1 -2; -3] := leftenv[i][-1 5; 4] * x[4 2; 1] *
                                     ham[i, j][5 -2; 2 3] * rightenv[j][1 3; -3]
        end
...
function ∂AC(x::MPSTensor, opp::MPOTensor, leftenv, rightenv)::typeof(x)
    @plansor toret[-1 -2; -3] := leftenv[-1 2; 1] * x[1 3; 4] * opp[2 -2; 3 5] *
                                 rightenv[4 5; -3]
end
