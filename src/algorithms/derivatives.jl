...
# mpo multiline
function ∂AC(x::RecursiveVec, operator, leftenv, rightenv)
    return RecursiveVec(circshift(map(t -> ∂AC(t...),
                                      zip(x.vecs, operator, leftenv, rightenv)),
                                  1))
end
...
function ∂AC2(x::RecursiveVec, operator1, operator2, leftenv, rightenv)
