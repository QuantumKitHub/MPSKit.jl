...
function propagator(A::AbstractFiniteMPS,z,ham::MPOHamiltonian, alg :: DynamicalDMRG{NaiveInvert};init=copy(A))
...
function propagator(A::AbstractFiniteMPS,z,ham::MPOHamiltonian, alg :: DynamicalDMRG{Jeckelmann};init=copy(A))
    w=real(z);eta=imag(z)
    ...
    while delta > alg.tol && numit < alg.maxiter
        ...
        for i in [1:(length(A)-1);length(A):-1:2]
            ...
            (res,convhist) = linsolve(H_AC,-eta*tos,init.AC[i],alg.solver,(eta*eta+w*w),1);
