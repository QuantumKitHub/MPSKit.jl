"""
    correlator(ψ, O₁, O₂, i, j)
    correlator(ψ, O₁₂, i, j)

Compute the 2-point correlator <ψ|O₁[i]O₂[j]|ψ> for inserting `O₁` at `i` and `O₂` at `j`.
...
function correlator(state::AbstractMPS, O₁::MPOTensor, O₂::MPOTensor, i::Int, js::AbstractRange{Int}) where {S}
    ...
    S₁ = _firstspace(O₁)
    S₁ == oneunit(S₁) ||
        throw(ArgumentError("O₁ should start with a trivial leg."))
    S₂ = _lastspace(O₂)
    S₂ == S₁' ||
        throw(ArgumentError("O₂ should end with a trivial leg."))
