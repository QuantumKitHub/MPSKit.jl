    ...
    for i in 1:odim
        ...
        if exci.trivial && isid(ham, i)
            @plansor start[i][-1 -2; -3 -4] -= start[i][1 4; -3 2] *
                                               r_RL(exci.right_gs)[2; 3] * τ[3 4; 5 1] *
                                               l_RL(exci.right_gs)[-1; 6] * τ[5 6; -4 -2]
        end
        ...
        if reduce(&, contains.(ham.data, i, i))
            if isid(ham, i)
                ...
                tm = TransferMatrix(exci.right_gs.AR, getindex.(ham.data, i, i),
                                    exci.left_gs.AL)
            end
...
function right_excitation_transfer_system(rBs, ham, exci; mom=exci.momentum,
    ...
    for i in odim:-1:1
        start = TransferMatrix(exci.left_gs.AL, ham[:], exci.right_gs.AR) * found *
                exp(1im * mom * len)
        ...
        if reduce(&, contains.(ham.data, i, i))
            if isid(ham, i)
                ...
                tm = TransferMatrix(exci.left_gs.AL, getindex.(ham.data, i, i),
                                    exci.right_gs.AR)
            end
