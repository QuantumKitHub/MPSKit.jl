...
function right_excitation_transfer_system(rBs, ham, exci; mom=exci.momentum, solver=Defaults.linearsolver)
    ...
    for i in odim:-1:1
        ...
        if exci.trivial && isid(ham,i)
            @plansor start[i][-1 -2;-3 -4] -= τ[6 2;3 4]*start[i][3 4;-3 5]*l_LR(exci.right_gs)[5;2]*r_LR(exci.right_gs)[-1;1]*τ[-2 -4;1 6]
        end
        ...
        if reduce(&,contains.(ham.data,i,i))
            if isid(ham,i)
                ...
                tm = TransferMatrix(exci.left_gs.AL,getindex.(ham.data,i,i),exci.right_gs.AR);
            end
