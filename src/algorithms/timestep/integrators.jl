...
function integrate(
    f::F, y₀, t₀, a, dt, method::ImplicitMidpoint
) where {F<:Union{MultipliedOperator,SumOfOperators}}
    y1, info = linsolve(x -> f(x, t₀ + dt / 2), y₀, y₀, 1, -0.5 * a * dt; tol=method.tol) #solve implicit problem
    return y1 + 0.5 * a * dt * f(y1, t₀ + dt / 2), info.converged, info
end
...
function integrate(
    ...
    return normalize(y), 1, nothing # normalize because RK4 does not conserve norm
end
