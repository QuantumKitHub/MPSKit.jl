...
function timestep(
    ...
    @sync for (loc, (ac, c)) in enumerate(zip(Ψ.AC, Ψ.CR))
        Threads.@spawn begin
            ...
            $temp_ACs[loc], converged, convhist = integrate(
                h_ac, $ac, $time, -1im, $timestep, alg.integrator
            )
            converged == 0 && @info "time evolving ac($loc) failed $(convhist.normres)"
        end

        Threads.@spawn begin
            h_c = MPSKit.∂∂C($loc, $Ψ, $H, $envs)
            $temp_CRs[loc], converged, convhist = integrate(
                h_c, $c, $time, -1im, $timestep, alg.integrator
            )
            converged == 0 && @info "time evolving ac($loc) failed $(convhist.normres)"
        end
    end
...
function timestep!(
    ...
    for i in 1:(length(Ψ) - 1)
        ...
        Ψ.CR[i], converged, convhist = integrate(
            h_c, Ψ.CR[i], t, 1im, dt / 2, alg.integrator
        )
        converged == 0 &&
    ...
    Ψ.AC[end], converged, convhist = integrate(
        h_ac, Ψ.AC[end], t, -1im, dt / 2, alg.integrator
    )
    converged == 0 &&
    ...
    for i in length(Ψ):-1:2
        ...
        Ψ.CR[i - 1], converged, convhist = integrate(
            h_c, Ψ.CR[i - 1], t + dt / 2, 1im, dt / 2, alg.integrator
        )
        converged == 0 &&
            @info "time evolving c($i) on the right->left sweep failed $(convhist.normres)"
    end
    ...
    h_ac = ∂∂AC(1, Ψ, H, envs)
    Ψ.AC[1], converged, convhist = integrate(
        h_ac, Ψ.AC[1], t + dt / 2, -1im, dt / 2, alg.integrator
    )
    converged == 0 &&
    ...
    for i in 1:(length(Ψ) - 1)
        ...
        if i != (length(Ψ) - 1)
            Ψ.AC[i + 1], converged, convhist = integrate(
                ∂∂AC(i + 1, Ψ, H, envs), Ψ.AC[i + 1], t, 1im, dt / 2, alg.integrator
            )
